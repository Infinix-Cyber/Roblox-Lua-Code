...
-- Кроссплатформенный метод отправки сообщений (Дельта / Ксено)
...
local function processCommand(text, sender)
    ...
    elseif text == "-# meshTool" or text == "-# meshtool" then
        isCommandRunning = true cache[text] = true
        if removeMeshFromCurrentTool(sender) then sendMsg("Tool mesh successfully cleared!")
        else sendMsg("Error: You must hold an item in your hand!") end
        task.delay(2, function() cache[text] = nil end)
        task.wait(0.5) isCommandRunning = false

    elseif text == "-# gethats" or text == "-# GetHats" then
        isCommandRunning = true cache[text] = true
        sendMsg("-gh 129462518582032 106249329428811 131385506535381 85392395166623 138364679836274 14160471787")
        task.wait(1.5)
        sendMsg("Hats successfully copied from Onyx Hub")
        task.delay(2, function() cache[text] = nil end)
        task.wait(0.5) isCommandRunning = false
    end
end

-- Хук чата для мобильной Delta
pcall(function()
    TextChatService.MessageReceived:Connect(function(msg)
        if msg.TextSource and not isBlocked and not isPanic and hasAccess then
            local senderId = msg.TextSource.UserId
            if (senderId == Player.UserId) or (senderId == FriendId) then
                local cmdPlayer = Players:GetPlayerByUserId(senderId)
                if cmdPlayer then processCommand(msg.Text, cmdPlayer) end
            end
        end
    end)
end)

-- Хук чата для ПК Xeno (через .Chatted)
local function setupChattedHook(p)
    if p.UserId == Player.UserId or p.UserId == FriendId then
        p.Chatted:Connect(function(msg)
            if not isBlocked and not isPanic and hasAccess then
...
task.spawn(function()
    for _, p in ipairs(Players:GetPlayers()) do setupChattedHook(p) end
    Players.PlayerAdded:Connect(setupChattedHook)
end)

local lines = {}
if not hasAccess then
    table.insert(lines, "========================================")
    table.insert(lines, "[ERROR] Access Denied!")
    table.insert(lines, "-- You do not have permission to use this script.")
    table.insert(lines, "========================================")
elseif isBlocked then
    table.insert(lines, "========================================")
    table.insert(lines, "[WARNING] Place is blacklisted!")
    table.insert(lines, "-- " .. gName .. " | Blocked!")
    table.insert(lines, "========================================")
else
    table.insert(lines, "========================================")
    table.insert(lines, "[Ping Tool] Loaded for: " .. gName)
    table.insert(lines, "[Info] User: " .. Player.Name .. " (" .. currentExecutor .. ")")
    table.insert(lines, "[Info] Developer: local maze")
    table.insert(lines, "[Info] Owner: " .. OwnerName)
    table.insert(lines, "[Info] Status: Early access players: " .. FriendName)
    table.insert(lines, "[Info] Date: July 27, 2026")
    table.insert(lines, "[Support] Discord: the.local.maze")
    table.insert(lines, "----------------------------------------")
    table.insert(lines, "[How to use]:")
    table.insert(lines, "Type in chat: -# ping")
    table.insert(lines, "Type in chat: -# fps")
    table.insert(lines, "Type in chat: -# meshTool")
    table.insert(lines, "Type in chat: -# meshBody [head/all/etc]")
    table.insert(lines, "Type in chat: -# GetHats")
    table.insert(lines, "========================================")
end
