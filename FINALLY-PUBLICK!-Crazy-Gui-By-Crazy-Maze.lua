...
local function gplr(String)
	...
	if strl == "all" then table.insert(Found, Players:GetPlayers())
...
Kill.MouseButton1Click:Connect(function()
	...
	if TargetList and TargetList then
		local Target = TargetList
		sendMsg("")
		_G.Killed = false
...
local function processCommand(text, sender)
    ...
    if string.sub(text, 1, 16) == "-# DisplayAttack:" then
        ...
        if targetPlayer then
            ...
            task.spawn(function()
                ...
                if not executed then pcall(function() Kill:Click() end) end
            end)
        end
    ...
    -- ОБНОВЛЕННАЯ КОМАНДА С ГЕЙМЕРСКОЙ ШУТКОЙ ПРО КАДРЫ
...
task.spawn(function()
    ...
    task.wait(3.0)
    pcall(function() sendMsg("This script is 100% custom made. Every single line of code and chat sorting logic was completely written by hand without using any chatgpt or gemini ai generators to keep the script optimization clean and fast.") end)
    task.wait(3.0)
    pcall(function() sendMsg("The brilliant concept and main design was originally borrowed from the old Local Maze profile. Then Crazy Maze took that pure layout idea, redesigned the core systems and fully reconstructed the script into this invisible chat remote.") end)
    task.wait(3.0)
    pcall(function() sendMsg("To use the client features type '-# ping' for server speed, '-# fps' for frame specs, '-# meshbody [part]' or '-# meshtool' to strip meshes and run '-# DisplayAttack:[Name]' with any item held to trigger the background kill method on targets.") end)
end)

print("Crazy Gui by Crazy Maze (Public Edition) успешно запущен.")
