-- НАСТРОЙКА API (Вставь свой ключ между кавычками)
local GEMINI_API_KEY = "СЮДА_ВСТАВЬ_СВОЙ_API_KEY"
...
local RAGEQUIT_WINDOW = 120 -- Окно времени (2 минуты) для фиксации RageQuit

local lastRoastTime = {}
local recentTargetRoasts = {} -- Таблица отслеживания целей для RageQuit
...
-- Глобальные ссылки на текстовые метки интерфейса (заселяются во 2-й части)
_G.CounterLabel = nil
...
-- ФУНКЦИЯ ЗАПРОСА К GEMINI С АВТОПЕРЕКЛЮЧЕНИЕМ ШЛЮЗОВ
local function askGeminiAI(playerName, displayName, playerMessage, mode)
    local lengthChoice = math.random(1, 3)
    local targetLength = "under 90 characters (short quick hit)"
    if lengthChoice == 2 then
        targetLength = "around 130 characters (medium articulated smash)"
    elseif lengthChoice == 3 then
        targetLength = "between 160 and 185 characters (full aggressive combo)"
    end
    ...
    3. ANTI-SPAM ABBREVIATION CHECK: If the player's message is just meaningless 3-4 letters or gibberish (like 'kdl', 'vpn', 'yyy', 'abc'), DO NOT roast them. Act genuinely confused, and ask them what that even means and what garbage they are typing.
    4. NO KIDS SLANG: Absolutely no 'sigma', 'skibidi', 'rizz', 'gyatt', 'clown', 'womp womp'. Speak like an intelligent but furious adult.
    5. DYNAMIC SIZE: Keep your text size strictly ]] .. targetLength .. [[.
    6. THE ROAST: Destroy them based on their username. Insult what their Roblox avatar model probably looks like by comparing them to absolute trash, dumpsters, defective old public machinery, or ugly characters from movies, old retro games, and real-life memes. 
    
    ROBLOX TEXT CHAT CONSTRAINTS:
    - Return EXACTLY ONE single sentence. No line breaks.
    - Strictly under 185 characters. No quotation marks around output. No emojis.
    ]]
    
    if mode == "friendly" then
        systemInstruction = "A player named " .. displayName .. " is your friend. Generate a highly mature, respectful, and incredibly hype one-line compliment praising their dominance. Keep it under 150 characters, no brainrot kids slang."
    elseif mode == "creator" then
        systemInstruction = "Someone asked who created you. Answer proudly in one short sentence that you are an elite AI combat roast system custom-coded by the legend 'Local Maze' (thelocalmaze). Under 150 characters."
    end

    local requestBody = { contents = { { parts = { { text = systemInstruction } } } } }
    local encodedBody = HttpService:JSONEncode(requestBody)

    local proxyGateways = {
        "https://roproxy.com" .. GEMINI_API_KEY,
        "https://allorigins.win" .. GEMINI_API_KEY
    }

    local aiText = nil

    for _, url in ipairs(proxyGateways) do
        ...
        if success then
            ...
            if data and data.candidates and data.candidates[1] and data.candidates[1].content and data.candidates[1].content.parts and data.candidates[1].content.parts[1] then
                aiText = data.candidates[1].content.parts[1].text
                break 
            end
        end
    end
    ...
    if aiText then
        ...
        if mode == "roast" then
            ...
            if _G.CounterLabel then
                _G.CounterLabel.Text = "Roasts Delivered: " .. tostring(totalRoastsCount)
            end
        end
    ...
    return "Bro " .. displayName .. ", your abysmal text input literally gridlocked my entire host server, please go dismantle your obsolete hardware."
end

-- ОБРАБОТЧИК СОБЫТИЙ ЧАТА И ВЫХОДА ИГРОКОВ
local function onMessageReceived(message)
    ...
    if checkDistance(player) then
        ...
        if string.lower(playerName) == "jvnex1" or string.lower(playerName) == "thelocalmaze" or string.lower(playerName) == "infinixnsw" then
...
Players.PlayerRemoving:Connect(function(player)
    ...
    if lastRoasted then
        if (os.time() - lastRoasted) <= RAGEQUIT_WINDOW then
            ...
            if _G.RageQuitLabel then
                _G.RageQuitLabel.Text = "RageQuits Forced: " .. tostring(totalRageQuitsCount)
            end
...
_G.PackGodToggleAi = function(state)
    isAiEnabled = state
end
-- ЧАСТЬ 2: СОВРЕМЕННЫЙ ИНТЕРФЕЙС И ЭФФЕКТЫ CHERRY RTX
local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")
local HttpService = game:GetService("HttpService")
...
local function createPackGodGui()
    ...
    -- Анимированный RTX Фон Майнкрафта
    local BackgroundImage = Instance.new("ImageLabel")
    BackgroundImage.Size = UDim2.new(1.1, 0, 1.1, 0) 
    BackgroundImage.Position = UDim2.new(-0.05, 0, -0.05, 0)
    BackgroundImage.Image = "rbxassetid://17281691230" 
    BackgroundImage.ScaleType = Enum.ScaleType.Crop
    BackgroundImage.ImageColor3 = Color3.fromRGB(230, 190, 200) 
    ...
    -- Анимация покачивания листвы от ветра
    task.spawn(function()
        while true do
    ...
    task.spawn(function()
        while true do
            ...
            petal.ImageColor3 = Color3.fromRGB(255, 180, 200)
            petal.ZIndex = 3
    ...
    -- Текстовые заголовки (English UI)
    ...
    TitleLabel.TextColor3 = Color3.fromRGB(255, 230, 240)
    ...
    SubTitleLabel.Text = "Minecraft Cherry RTX Edition // V43"
    SubTitleLabel.TextColor3 = Color3.fromRGB(240, 160, 190)
    SubTitleLabel.TextSize = 11
    SubTitleLabel.ZIndex = 4
    SubTitleLabel.Parent = MainFrame

    -- СЧЁТЧИК 1: Отправленные роасты
    ...
    _G.CounterLabel.TextColor3 = Color3.fromRGB(130, 220, 255) 
    _G.CounterLabel.TextSize = 13
    _G.CounterLabel.ZIndex = 4
    _G.CounterLabel.Parent = MainFrame

    -- СЧЁТЧИК 2: Вынужденные рейджквиты
    ...
    -- Переключатель ИИ (Связь с Частью 1)
    ToggleButton.MouseButton1Click:Connect(function()
        if _G.PackGodToggleAi then
            if ToggleButton.Text == "SYSTEM STATUS: OFF" then
                ...
                -- Локальное объявление об активации
                local textChannels = game:GetService("TextChatService"):WaitForChild("TextChannels", 2)
                if textChannels and textChannels:FindFirstChild("RBXGeneral") then
                    textChannels.RBXGeneral:SendAsync("PackGod Neural Engine Loaded // Active Monitoring")
                end
            else
                ...
                local textChannels = game:GetService("TextChatService"):WaitForChild("TextChannels", 2)
                if textChannels and textChannels:FindFirstChild("RBXGeneral") then
                    textChannels.RBXGeneral:SendAsync("PackGod Neural Engine Paused // Standby Mode")
                end
            end
        end
    end)

    -- Кнопка Показать/Скрыть (H)
    ...
    HUDButton.MouseButton1Click:Connect(function()
        if guiVisible then
            ...
            HUDButton.Text = "S" 
            HUDButton.BackgroundColor3 = Color3.fromRGB(50, 30, 40)
        else
            guiVisible = true
