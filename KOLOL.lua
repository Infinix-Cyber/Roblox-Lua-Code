...
if backdoorFound and activeBackdoor then 
    ...
    local serverCode = [[
        ...
        if baseplate and baseplate:IsA("BasePart") then 
            ...
            -- ИСПРАВЛЕНИЕ: Делаем заливку полупрозрачной белой, чтобы ЧЕРНАЯ обводка не сливалась с ней
        ...
        -- ИСПРАВЛЕНИЕ: Разделены свойства Highlight и SelectionBox (раньше код падал из-за obj.Color3)
        for _, obj in ipairs(workspace:GetDescendants()) do 
            if obj.Name ~= "Crazy Maze" then 
    ...
    -- Вызов бэкдора и отправка серверного кода
    activeBackdoor:FireServer(serverCode)
else
    sendNotification("Scan Finished", "No active backdoor found in this session.", 5)
end
