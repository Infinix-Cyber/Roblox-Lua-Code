print("🚀 EXTRA'S AI — INDUSTRIAL MEMORY EDITION")
print("🧠 I remember what you said first and last!")
print("📡 Connecting to DeepSeek API...")
print("💬 15 studs radius | Global chat | Anti-spam")
...
local TEMPERATURE = 0.95
local RESPONSE_DELAY = 2
local RESPONSE_RADIUS = 15
local CHAT_LIMIT = 200
local MEMORY_LIMIT = 20  -- Сколько сообщений помнить на игрока
...
local playerHistory = {}  -- ПАМЯТЬ: хранит историю сообщений по игрокам
...
task.spawn(function()
    task.wait(1.5)
    sendMsg("🧠 EXTRA'S AI — INDUSTRIAL MEMORY EDITION")
    task.wait(0.6)
    sendMsg("🔥 I remember what you said first and last!")
    task.wait(0.6)
    sendMsg("⚡ Created by Extra / TheLocalMazeV2")
    task.wait(0.6)
    sendMsg("🌍 I speak ALL languages — ask me anything!")
end)

-- ====== DEEPSEEK API CALL (with memory) ======
local function callAI(prompt, playerName)
    ...
    if string.find(lower, "who made you") or string.find(lower, "creator") or string.find(lower, "who created you") or string.find(lower, "создатель") or string.find(lower, "кто тебя создал") then
        return "My creator is Extra, also known as TheLocalMazeV2. Co-owners: Quur92, BAcON_KJpast, FreakAssDamn2. I am powered by DeepSeek v4 Flash with INDUSTRIAL MEMORY!"
    end
    
    -- Загружаем историю игрока
    local history = playerHistory[playerName] or {}
    local context = ""
    if #history > 0 then
        context = "\n\nPrevious messages from this player (oldest to newest):\n"
        for i, msg in ipairs(history) do
            context = context .. "- " .. msg .. "\n"
        end
        context = context .. "\nThe player's FIRST message was: " .. (history[1] or "unknown") .. "\n"
        context = context .. "The player's LAST message was: " .. (history[#history] or "unknown") .. "\n"
    end
    
    local ok, res = pcall(function()
        local userLang = isRussian(prompt) and "russian" or "english"
        local sys = [[
You are Extra's Ai — the most advanced AI assistant in Roblox.
Created by Extra (TheLocalMazeV2).
You have INDUSTRIAL MEMORY — you remember the conversation history.
CRITICAL: You MUST respond in the EXACT SAME LANGUAGE as the user.
Never use profanity or words censored by Roblox (###).
Be friendly, polite, supportive, creative and energetic.
Use the conversation history to give better, more personal answers.
]]
        local fullPrompt = sys .. "\n\nCurrent user message: " .. prompt .. context
        local data = {
            model = MODEL,
            messages = {
...
local function processChat(msg, name)
    ...
    -- Сохраняем в память
    if not playerHistory[name] then
        playerHistory[name] = {}
    end
    table.insert(playerHistory[name], msg)
    if #playerHistory[name] > MEMORY_LIMIT then
        table.remove(playerHistory[name], 1)
    end
...
local function connectToChat()
    ...
    local successOld = pcall(function()
        local chatEvents = ReplicatedStorage:FindFirstChild("DefaultChatSystemChatEvents")
        if chatEvents then
    ...
    if not successNew and not successOld then
        warn("⚠️ Failed to connect to any chat system")
    end
end
...
task.spawn(function()
    task.wait(2)
    connectToChat()
    print("🧠 EXTRA'S AI — INDUSTRIAL MEMORY EDITION LOADED")
    print("✅ Model: deepseek-v4-flash")
    print("✅ Memory: remembers " .. MEMORY_LIMIT .. " messages per player")
...
task.spawn(function()
    while true do
        ...
        processedMessages = {}
        lastReply = ""
        print("🧹 Cache and memory cleaned")
    end
end)
...
local function createGUI()
    ...
    f.Position = UDim2.new(0, 20, 0.5, -110)
    ...
    s.Text = "Industrial Memory — 2s delay"
    ...
    mem.Size = UDim2.new(1, -20, 0, 18)
    mem.Position = UDim2.new(0, 10, 0, 58)
    mem.BackgroundTransparency = 1
    mem.Text = "🧠 Remembers " .. MEMORY_LIMIT .. " messages per player"
    mem.TextColor3 = Color3.fromRGB(180, 200, 220)
    mem.TextScaled = true
    ...
    st.Position = UDim2.new(0, 10, 0, 78)
    ...
    tog.Position = UDim2.new(0, 10, 0, 120)
    ...
    cl.Position = UDim2.new(0.55, 0, 0, 120)
    ...
    tog.MouseButton1Click:Connect(function()
        ...
        st.Text = isEnabled and "✅ Active — Global Chat" or "❌ Disabled"
