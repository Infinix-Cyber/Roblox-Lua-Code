...
TextChatService.MessageReceived:Connect(function(message)
	-- Проверяем, что сообщение отправил реальный игрок, а не система
	if not message.FromUser or typeof(message.FromUser) ~= "Instance" then return end
	
	-- Дополнительная проверка, что этот Instance — действительно игрок
	if not message.FromUser:IsA("Player") then return end

	local username = message.FromUser.Name
	if not WHITELIST[username] then return end
