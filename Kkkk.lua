-- =====================================================
-- GOON TRANSLATOR V2.4 (FULLY FIXED - NO NIL)
...
local function sendToChat(msg)
    ...
    -- Способ 3: Прямая эмуляция ввода
    ...
    if localPlayer and localPlayer:FindFirstChild("PlayerGui") then
        ...
        if chatGui and chatGui:FindFirstChild("Frame") then
            local chatBar = chatGui.Frame:FindFirstChild("ChatBarParentFrame")
            if chatBar and chatBar:FindFirstChild("ChatBar") then
                pcall(function()
                    chatBar.ChatBar.Text = msg
                    chatBar.ChatBar:SendMessage()
                end)
                return
            end
        end
...
ttsBtn.Size = UDim2.new(0.20, 0, 1, 0)
ttsBtn.Position = UDim2.new(0.78, 0, 0, 0)
ttsBtn.BackgroundColor3 = Color3.fromRGB(160, 80, 200)
ttsBtn.Text = "🔊 TTS"
ttsBtn.TextColor3 = Color3.fromRGB(255,255,255)
ttsBtn.TextSize = 18
ttsBtn.Font = Enum.Font.GothamBold
ttsBtn.BorderSizePixel = 0
ttsBtn.Parent = actionPanel
local ttsCorner = Instance.new("UICorner")
ttsCorner.CornerRadius = UDim.new(0, 12)
ttsCorner.Parent = ttsBtn
