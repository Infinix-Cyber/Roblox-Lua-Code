print("Extra's Ai (Modified by Quur92) - GLOBAL CHAT MODE")
...
-- ====== ОТПРАВКА В ГЛОБАЛЬНЫЙ ЧАТ (КАК В ПРИМЕРЕ) ======
...
-- ====== ПРИВЕТСТВИЕ В ГЛОБАЛЬНЫЙ ЧАТ ======
task.spawn(function()
    task.wait(1.5)
    sendMsg("Extra's Ai activated! I speak all languages.")
    task.wait(0.6)
...
local function processChat(msg, name)
    ...
    task.spawn(function()
        ...
        if reply then
            ...
            print("Sent reply to global chat")
        end
    end)
end

-- ====== ПОДКЛЮЧЕНИЕ К ЧАТУ (КАК В ПРИМЕРЕ) ======
...
task.spawn(function()
    task.wait(2)
    connectToChat()
    print("Extra's Ai ready - GLOBAL CHAT MODE")
    print("All players will see AI messages")
...
local function createGUI()
    ...
    s.Text = "2026 - Gemini 2.0 Flash - GLOBAL CHAT"
    ...
    st.Text = "Active - all players see messages"
...
print("Extra's Ai loaded - GLOBAL CHAT MODE")
print("All messages go to global chat")
print("Type /ai on / off")
