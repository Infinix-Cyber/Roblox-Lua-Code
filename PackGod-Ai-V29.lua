...
-- МАКСИМАЛЬHО РАСШИРЕHHАЯ БАЗА С РАЗНЫМИ МАГАЗИНАМИ И БРЕНДАМИ
local MATRIX_BANKS = {
    layer1_intros = {
    ...
    layer2_punches = {
        "typing in all caps like your keyboard was bought from a Dollar Tree clearance box ",
    ...
    layer5_conclusions = {
        "this whole display is a certified multi-brand comedy show.",
        "you are genuinely the funniest entity on my screen right now.",
        "that is an absolute legendary performance of pure goofy discount behavior.",
        "I have never seen someone try this hard and look this incredibly cheap.",
        "go back to the lobby and restart because your whole style is a budget disaster."
    },
    short_conclusions = {
...
sayInChat("PackGod LocalAI V29 [Brand Chaos Engine]")
...
local function generateChaosMatrixRoast(displayName, forceLong)
    ...
    if sizeStyle == 1 then
        local rawText = nameIntro .. shortConclusion
        if string.len(rawText) > 58 then rawText = string.sub(rawText, 1, 55) .. "..." end
        return rawText
    elseif sizeStyle == 2 then
        local rawText = nameIntro .. p2 .. "Total comedy show."
        ...
        if currentLength < 175 then
            local padding = " This is a total certified liquidation sale."
