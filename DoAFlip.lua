		...
		if LocalPlayer.Character.PrimaryPart ~= nil and Player.Character and Player.Character.PrimaryPart ~= nil then
			...
			if LocalPlayer.Character and Victim and Victim.PrimaryPart ~= nil then
				...
				if Tool ~= nil then
					...
					repeat
						wait()
					until not Tool or Tool and (Tool.Parent == Workspace or Tool.Parent == Victim)

					Victim.HumanoidRootPart.Anchored = false
					wait(.12)

					local humToKill = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
					if humToKill then
						humToKill.Health = 0
					end
					...
					if Victim and Victim:FindFirstChildOfClass("Humanoid") then
						if Victim.Humanoid.Health <= 15 then
							game:GetService("StarterGui"):SetCore("SendNotification", {
								Title = "Tool Dupe";
								Text = "The requested user has been killed!";
							})
						else
							wait(.55)
